...
| **Compile-probe result** (`cargo check --target aarch64-linux-android`) | Predicted first errors: `lib.rs:94`, `lib.rs:97-101`, `ytdlp_setup.rs:32-76`. | **RUN — CONFIRMED with one divergence** (2026-07-27, rustc 1.97.1, NDK 27.1.12297006, full log in `cargo-check-android.log`): all three predictions are real and are the only Android-specific compile errors — `lib.rs:94` E0433 (`tauri_plugin_window_state`), `lib.rs:99-100` E0433 (`tauri_plugin_updater`, `tauri_plugin_process`), `ytdlp_setup.rs:32,55` E0308 (no Android cfg arm → `()` where `&str` expected). Divergence: one additional error, `http_api/frontend.rs:22,31` E0599 — `rust_embed`'s `FrontendAssets::get` missing because `../dist/` didn't exist at compile time; environmental (frontend not yet built), not Android-specific, resolved by `pnpm build:frontend`. Notable positive: the **entire dependency tree — tauri 2.7, sqlx/libsqlite3-sys, axum, reqwest — compiled clean for `aarch64-linux-android`**; only the app crate itself fails. |
